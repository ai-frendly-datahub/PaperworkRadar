...
def handle_doc_checklist(*, db_path: Path, days: int = 30, limit: int = 10) -> str:
    """Generate a document checklist from recent paperwork-related articles."""
    cutoff = datetime.now(tz=UTC) - timedelta(days=days)
    conn = duckdb.connect(str(db_path), read_only=True)
    try:
        rows = conn.execute(
    ...
    for title, source, entities_json, collected_at in rows:
        ...
        for entity_name in entities:
            if entity_name not in checklist:
                checklist[entity_name] = []
            if len(checklist[entity_name]) < limit:
                checklist[entity_name].append(f"{title} ({source})")

    if not checklist:
        return "No document checklist items found for the recent period."

    lines = [f"Document checklist (last {days} days):"]
...
def handle_quality_report(
    *,
    db_path: Path,
    category: str = "paperwork",
    categories_dir: Path | None = None,
    days: int = 30,
    limit: int = 500,
) -> str:
    """Build a structured quality report from recent stored paperwork events."""
    try:

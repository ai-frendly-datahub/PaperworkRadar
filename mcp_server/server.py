...
def _db_path() -> Path:
    return Path(os.getenv("RADAR_DB_PATH", "data/paperwork.duckdb"))
...
def _list_tool_specs() -> list[dict[str, object]]:
    return [
        {
            "name": "search",
            "description": "Search government paperwork and forms.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "query": {"type": "string"},
        ...
        {
            "name": "recent_updates",
            "description": "List recently collected paperwork articles from DuckDB.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "top_trends",
            "description": "Show top entity frequencies from recent paperwork article matches.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "days": {"type": "integer", "minimum": 1},
...
def create_app() -> _McpApp:
    ...
    app = server_ctor("paperwork-radar")

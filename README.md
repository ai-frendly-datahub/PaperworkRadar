...
- **데이터 수집**: 정부24 API (gov.kr)
- **엔티티 분석**: 서류 유형별 키워드 매칭 (신분증, 여권, 면허증 등)
- **트렌드 리포트**: DuckDB 저장 + HTML 리포트로 서류/행정 동향 시각화
...
- 워크플로: `.github/workflows/radar-crawler.yml`
  ...
  - 환경 변수 `RADAR_CATEGORY` 값은 `paperwork`입니다.
...
- 설정 방법:
  ...
  3) 워크플로 파일의 `RADAR_CATEGORY`가 `paperwork`인지 확인
